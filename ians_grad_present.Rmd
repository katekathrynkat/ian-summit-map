...
# Headers
legend_title <- data.frame(x = c(-1846000, -1871000),
                           y = c(2812000, 2839000),
...
# Place info text
legend2 <- data.frame(x = c(rep(-1898000, 4), rep(-1860000, 4)),
...
# Place info icons
legend_pics <- data.frame(x = c(rep(-1901000, 4), rep(-1863000, 4)),
                          y = c(rep(seq(2834000, 2819000, length.out = 4), 2)),
...
ggplot() +
 ...
 geom_raster(data = hillshade_df,
  ...
  geom_image(data = uw,
             ...
             size = 0.02, asp = 0.75) +
  ### LEGEND ###
 geom_rect(aes(xmin = -1908000, xmax = -1778000, ymin = 2708000, ymax = 2871000),
           fill = 'white', alpha = 0.3) +
  geom_rect(aes(xmin = -1908000, xmax = -1778000, ymin = 2708000, ymax = 2871000),
            fill = NA, color = 'midnightblue', size = 2) +
 geom_text(aes(x = -1843000, y = 2866000, label = 'STOKE IS HIGH'),
           size = 12, color = 'black', fontface = 2, family = 'serif') +
 geom_text(aes(x = -1843000, y = 2860000, label = 'IAN CULHANE\'S SUMMITS IN WASHINGTON & OREGON, 2015-2019'),
           size = 7, color = 'black', fontface = 2, family = 'serif') +
 geom_text(aes(x = -1843000, y = 2850000, label = 'Volcanic peaks, spicy scrambles, stormy bails, trashy hikes\u2014and everything in between. This map shows most of the notable and ridiculous\ntrips that Ian Culhane embarked upon during his four years as an undergraduate at the University of Washington. This map doesn\'t show the\nstories behind the trips: ice axes buzzing with electricity during a stormed-out bivvy on Goode Mountain; an entire taco bar and cheesecake\nhauled up to Three Fingers Lookout; or a glorious Christmas Eve summit of the Mt. Ellinor trailhead, after which Ian\'s sister drove their\nparents\' car into a ditch. Here, these anecdotes are just points on a map; if you want to know the full stories you\'ll have to ask Ian himself.'),
 ...
 geom_image(aes(x = -1901000, y = 2834000, image = './pics/uwlogo.png'),
            size = 0.01, asp = 0.75) +
 geom_text(aes(x = -1780000, y = 2713000, label = 'Map by Kate Culhane, 2019.\nElevation data from the USGS 1997-2014 Edition of the National Atlas of the United States, Alber\'s projection, NAD83.\nCode at github.com/katekathrynkat/ian-summit-map.'),
           size = 3, color = 'black', hjust = 1, fontface = 4) +
 # annotation_north_arrow(location = 'br', which_north = "true",
 #                        style = north_arrow_fancy_orienteering) +
 # annotation_scale(location = 'br', pad_y = unit(0, 'cm')) +
 geom_segment(aes(x = -1817000, y = 2828000,
                  xend = -1817000+16214, yend = 2828000),
              color = 'midnightblue', size = 5) +
 geom_segment(aes(x = -1817000+16214, y = 2828000,
                  xend = -1817000+16214+16214, yend = 2828000),
              color = 'gold', size = 5) +
 geom_text(aes(x = -1817000, y = 2831000, label = '0'),
            size = 5, color = 'black', fontface = 2) +
  geom_text(aes(x = -1817000+16214, y = 2831000, label = '10'),
            size = 5, color = 'black', fontface = 2) +
  geom_text(aes(x = -1817000+16214+16214+1500, y = 2831000, label = '20 mi.'),
            size = 5, color = 'black', fontface = 2) +
  geom_image(aes(x = -1827000, y = 2828000, image = './pics/rose_14deg.png'),
             size = 0.06, asp = 0.75) +
  ### THEME ###
  theme_void() +
  theme(legend.position = 'none',
        plot.margin = unit(c(0,0,0,0), 'cm'),
        panel.border = element_rect(color = 'midnightblue', size = 15, fill = NA)) +
  scale_x_continuous(expand = c(0,0,0,0)) +
  scale_y_continuous(expand = c(0,0,0,0)) +
  coord_cartesian()


ggsave('map6.pdf', width = 30, height = 40)
